...
<div class="container mt-5">
    <div class="card shadow-sm">
        <div class="card-header bg-primary text-white">
            <h4 class="mb-0">Edit Peristiwa Kelahiran</h4>
        </div>
        <div class="card-body">
            ...
            <form action="{{ route('kelahiran.update', $kelahiran->kelahiran_id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="anak_warga_id" class="form-select" required>
                            ...
                            @foreach($warga as $w)
                                <option value="{{ $w->warga_id }}" {{ (old('anak_warga_id', $kelahiran->anak_warga_id) == $w->warga_id) ? 'selected' : '' }}>
                                    {{ $w->nama }} ({{ $w->no_ktp }})
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select name="ayah_warga_id" class="form-select" required>
                            ...
                            @foreach($warga as $w)
                                @if($w->jenis_kelamin == 'L')
                                    <option value="{{ $w->warga_id }}" {{ (old('ayah_warga_id', $kelahiran->ayah_warga_id) == $w->warga_id) ? 'selected' : '' }}>
                                        {{ $w->nama }}
                                    </option>
                                @endif
                            @endforeach
                        ...
                        <select name="ibu_warga_id" class="form-select" required>
                            ...
                            @foreach($warga as $w)
                                @if($w->jenis_kelamin == 'P')
                                    <option value="{{ $w->warga_id }}" {{ (old('ibu_warga_id', $kelahiran->ibu_warga_id) == $w->warga_id) ? 'selected' : '' }}>
                                        {{ $w->nama }}
                                    </option>
                                @endif
                            @endforeach
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="date" name="tanggal_lahir" class="form-control" value="{{ old('tanggal_lahir', $kelahiran->tanggal_lahir) }}" required>
                    </div>
                    ...
                    <div class="col-md-12">
                        ...
                        @if($kelahiran->foto && $kelahiran->foto->first())
                            <small>Foto saat ini: <img src="{{ asset('storage/'.$kelahiran->foto->first()->file_url) }}" width="80"></small>
                        @endif
                    </div>
