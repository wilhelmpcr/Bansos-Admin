                ...
                <div class="row">
                    ...
                    <div class="col-md-3">
                        <select name="status_seleksi" class="form-select" onchange="this.form.submit()">
                            ...
                            <option value="ditolak" {{ request('status_seleksi') == 'ditolak' ? 'selected' : '' }}>Ditolak
                            </option>
                            <option value="diterima" {{ request('status_seleksi') == 'diterima' ? 'selected' : '' }}>
                                Diterima
                            </option>
                            <option value="dipending" {{ request('status_seleksi') == 'dipending' ? 'selected' : '' }}>
                                Dipending</option>
                        </select>
                    </div>
                    ...
                    @forelse($pendaftar as $index => $p)
                        <tr>
                            ...
                            <td class="text-center">
                                <span
                                    class="badge
            @if ($p->status_seleksi == 'diterima') bg-success
            @elseif($p->status_seleksi == 'ditolak') bg-danger
            @else bg-warning @endif">
                            ...
                            <td class="text-center">
                                <div class="btn-group" role="group">
                                    <a href="{{ route('pendaftar_bantuan.show', $p->pendaftar_id) }}"
                                        class="btn btn-sm btn-info">
                                        Detail
                                    </a>

                                    <a href="{{ route('pendaftar_bantuan.edit', $p->pendaftar_id) }}"
